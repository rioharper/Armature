---
name: armature-spec
description: Turn a concept — an armature-pitch brief, or a described idea — into a defensible engineering spec through a skeptical technical interview covering architecture, kinematics, feasibility, and parts. Use when the user wants to design or "spec out" a robot, mechanism, or subsystem; wants a trade study or design review of an electromechanical design or proposed change; or wants spec questions drafted into a questionnaire for a third party who holds the answers. An idea still at the "worth building, and for whom" stage goes to armature-pitch first — this skill assumes that's settled and goes deep on the engineering.
---
...
1. **Mission** — What must the robot *do*, stated as observable outcomes, not mechanisms? ("Pick tomatoes" not "have a gripper.") What does success look like, quantitatively? Cycle time, payload, accuracy, uptime? If a concept brief exists, translate each of its RC-numbered outcomes into one or more verifiable REQ-0xx (with a method) — confirm the numbers, don't relitigate the audience.
...
If the design space feels stale or the requirements are unusually hard, dispatch the **armature-inventor** agent — one per idea family, each prompt carrying the one-sentence design tension and the constraint numbers, in waves of two or three per the plugin's `references/subagent-dispatch.md` (two levels above this skill). Run the filter and the boring-baseline comparison here with the user when the briefs come back.
...
- **Ask first, hunt second.** Request datasheets the user already has. For anything missing, dispatch the **armature-librarian** agent with the exact P/N (or the description plus the specs that matter): a pre-confirmed P/N is cached in the run; a candidate waits in the agent's staging file for your confirmation with the user, then its Merge step. A dispatch that canvasses a field rather than a part — custom vendors, stock catalogues — leaves a permanent survey record in `docs/datasheets/` whether or not it confirms anything. Cite index rows, never memory, and treat a CANDIDATE row as open until the user confirms it. Several parts at once go out in waves of two or three per the plugin's `references/subagent-dispatch.md` (two levels above this skill).
- **When a number can't be sourced, stop and say so.** If a design-critical spec (stall torque, continuous current, rotor inertia, yield strength, max operating temperature) isn't available from the user or a trustworthy public source, log it as an open question and pause for the user rather than inventing a plausible value — and when a person the user can reach holds it (a vendor's application engineer), offer a **questionnaire**.
